...
func (self *TP) send(data *NetData) {
	...
	if err != nil {
		debugPrintln("Debug: 发送数据-编码出错", err)
		return
	}
	conn := self.getConn(data.To)
	if conn == nil {
		debugPrintf("Debug: 发送数据-连接已断开: %+v", data)
		return
	}
	// 封包
	end := self.Packet(d)
	// 发送
	conn.Write(end)
	debugPrintf("Debug: 发送数据-成功: %+v", data)
}

// 解码收到的数据并存入缓存
func (self *TP) save(conn *Connect) {
	debugPrintf("Debug: 收到数据-Byte: %v", conn.TmpBuffer)
	// 解包
	dataSlice := make([][]byte, 10)
	dataSlice, conn.TmpBuffer = self.Unpack(conn.TmpBuffer)

	for _, data := range dataSlice {
		debugPrintf("Debug: 收到数据-解码前: %v", string(data))
		...
		if err == nil {
			...
			// 添加到读取缓存
			self.apiReadChan <- d
			debugPrintf("Debug: 收到数据-NetData: %+v", d)
		} else {
			debugPrintf("Debug: 收到数据-解码错误: %v", err)
		}
	}
}
