...
retry:
	...
	if err != nil {
		debugPrintf("Debug: 监听端口出错: %v", err)
	...
	for self.listener != nil {
		...
		debugPrintf("Debug: 客户端 %v 已连接，但尚未验证身份！", conn.RemoteAddr().String())
...
func (self *TP) sInitConn(conn *Connect, remoteAddr string) (nodeuid string, usable bool) {
	...
	for i, data := range dataSlice {
		debugPrintln("Debug: 收到数据-第1批-解码前: ", string(data))
		...
		if i == 0 {
			debugPrintf("Debug: 收到数据-第1条-NetData: %+v", d)
